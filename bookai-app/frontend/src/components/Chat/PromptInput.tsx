...
export default function PromptInput() {
  const { prompt, mode, strategy, temperature, model, loading, setPrompt, setLoading, setResponse, setError, addToHistory } =
    useAppStore()
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const { answer, inputTokens, outputTokens, responseTimeMs, costUsd } = await sendRequest(prompt, mode, strategy, temperature, model)
      setResponse(answer, inputTokens, outputTokens, responseTimeMs, costUsd)
      addToHistory({
